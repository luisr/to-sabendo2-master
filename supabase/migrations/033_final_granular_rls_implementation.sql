-- =============================================================================
--  MIGRAÇÃO 033: IMPLEMENTAÇÃO FINAL DA ARQUITETURA DE RLS GRANULAR
...
DROP POLICY IF EXISTS "Admins e Gerentes podem gerenciar os status" ON public.task_statuses;
...
CREATE POLICY "Gerentes podem gerenciar projetos" ON public.projects
FOR ALL USING (public.is_admin() OR public.is_project_manager(id));

-- Para 'tasks'
CREATE POLICY "Membros podem ver tarefas" ON public.tasks
...
WITH CHECK (public.is_admin() OR (public.is_project_member(project_id) AND assignee_id = auth.uid()));
