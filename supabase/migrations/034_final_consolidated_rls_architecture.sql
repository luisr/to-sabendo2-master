-- =============================================================================
--  MIGRAÇÃO 034: IMPLEMENTAÇÃO FINAL DA ARQUITETURA DE RLS GRANULAR (CONSOLIDADO)
--  Este é o script mestre e definitivo. Ele limpa todas as políticas antigas,
--  recria as funções auxiliares com o contexto de segurança correto
--  (SECURITY DEFINER) e implementa as políticas granulares para cada perfil.
-- =============================================================================
...
DROP POLICY IF EXISTS "Membros podem ver tarefas" ON public.tasks;
...
-- Políticas da tabela 'collaborators'
...
-- Estas funções agora podem ler tabelas sem acionar as RLS dessas tabelas.
...
-- Para 'tasks':
CREATE POLICY "Membros podem ver tarefas do projeto" ON public.tasks
FOR SELECT USING (public.is_admin() OR public.is_project_member(project_id));

CREATE POLICY "Membros podem atualizar o status/progresso das suas tarefas" ON public.tasks
FOR UPDATE USING (public.is_admin() OR (public.is_project_member(project_id) AND assignee_id = auth.uid()));

CREATE POLICY "Gerentes podem gerenciar todas as tarefas do projeto" ON public.tasks
...
CREATE POLICY "Gerentes podem gerenciar a equipe do projeto" ON public.collaborators
FOR ALL USING (public.is_admin() OR public.is_project_manager(project_id));
