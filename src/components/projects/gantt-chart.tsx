...
import { startOfToday, startOfWeek, endOfWeek, subDays, startOfMonth, endOfMonth, subMonths, startOfYear, endOfYear, getMonth, isWithinInterval, parseISO, differenceInHours } from 'date-fns';
...
const getInterval = (filter: TimeFilter): Interval | null => {
    ...
    switch (filter) {
        ...
        case 'quarter': return { start: subMonths(today, 3), end: endOfToday() };
        case 'semester':
            const currentMonth = getMonth(today);
            return currentMonth < 6 
                ? { start: new Date(currentYear, 0, 1), end: new Date(currentYear, 5, 30) } 
                : { start: new Date(currentYear, 6, 1), end: new Date(currentYear, 11, 31) };
...
const filterTasksByInterval = (tasks: TaskNode[], interval: Interval): TaskNode[] => {
    return tasks.map(task => {
        ...
        if (isInInterval || filteredChildren.length > 0) {
            return { ...task, children: filteredChildren, subtasks: filteredChildren };
        }
        return null;
...
export default function GanttChart({ tasks, isConsolidated }: { tasks: TaskNode[], isConsolidated: boolean }) {
    ...
    const ganttTasks = useMemo(() => {
        ...
        const processNode = (node: TaskNode | { id: string, name: string, children: TaskNode[] }, level: number, isProjectHeader = false) => {
            ...
            flattened.push({
                id: nodeId, name: `${icon} ${node.name || 'Nome indefinido'}`,
                start: (node as TaskNode).start_date || '', end: (node as TaskNode).end_date || '',
    ...
    useEffect(() => {
        ...
        if (ganttRef.current && ganttTasks.length > 0) {
            ...
            gantt = new Gantt(ganttRef.current, ganttTasks, {
            ...
            if (ganttStartDate && today >= ganttStartDate && todayLineRef.current) {
                const hoursFromStart = differenceInHours(today, ganttStartDate);
                const leftPos = hoursFromStart * (gantt.options.column_width / gantt.options.step);
    ...
    return (
        <div className="w-full h-full gantt-container flex flex-col">
            ...
            <div className="flex-1 overflow-auto relative">
                {tasksLoading ? <div className="flex items-center justify-center h-full"><Loader2 className="h-8 w-8 animate-spin" /></div>
                : ganttTasks.length > 0 ? (
